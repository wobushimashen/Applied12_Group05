...
class AdminMenu:
    ...
    def _view_all_rooms(self):
        ...
        print(f"  {'ID':<8} {'Room':<12} {'Type':<8} {'Building':<10} {'Cap':<5} "
              f"{'Price/hr':<9} {'Hours':<11} {'Available'}")
        print("  " + "-" * 86)
        for room in rooms:
            bname = buildings.get(room.building_id, "Unknown")
            avail = "Yes" if room.is_available else "No"
            hours = f"{room.opening_time}-{room.closing_time}"
            print(f"  {room.room_id:<8} {room.room_name:<12} {room.room_type:<8} "
                  f"{bname:<10} {room.capacity:<5} ${room.price_per_hour:<8.2f} "
                  f"{hours:<11} {avail}")
    ...
    def _create_room(self):
        ...
        room_type = input(">> Room Type (Small/Medium/Large): ").strip() or "Small"
        capacity = input(">> Capacity (leave blank for type default): ").strip()

        success, result = self.room_service.create_room(room_name, building_name, capacity, room_type)
        if success:
            room = result
    ...
    def _update_room(self):
        ...
        new_standard_equipment = input(">> New Standard Equipment (leave blank to keep): ").strip()

        print(f"  Current Opening Time: {room.opening_time}")
        new_opening_time = input(">> New Opening Time HH:MM (leave blank to keep): ").strip()

        print(f"  Current Closing Time: {room.closing_time}")
        new_closing_time = input(">> New Closing Time HH:MM (leave blank to keep): ").strip()
        ...
        success, result = self.room_service.update_room(
            ...
            new_standard_equipment=new_standard_equipment or None,
            new_opening_time=new_opening_time or None,
            new_closing_time=new_closing_time or None,
        )
        if success:
            room = result
